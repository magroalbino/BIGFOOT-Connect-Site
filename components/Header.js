...
// Mascara e-mail para exibição mais privada
...
export default function Header() {
  ...
  // Marca como montado no cliente (evita SSR)
  useEffect(() => {
    setMounted(true);
  }, []);

  // Inicialização do tema (somente no cliente)
  useEffect(() => {
  ...
  // Logout (limpa apenas dados do usuário, preserva tema/idioma)
  const handleLogout = useCallback(async () => {
    try {
      // Remove apenas chaves específicas da aplicação
      const keysToRemove = ['email', 'token', 'userData', 'profile'];
  ...
  // Toggle tema (com verificação SSR)
  ...
  // Evita renderização completa até o componente estar montado no cliente
  if (!mounted) {
    return (
      <header className="bg-gradient-to-r from-gray-900 to-gray-800 border-b-2 border-gray-800 shadow-lg sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-gray-700 rounded-lg animate-pulse" />
              <div className="w-32 h-6 bg-gray-700 rounded animate-pulse hidden sm:block" />
            </div>
          </div>
        </div>
      </header>
