...
function truncateAddress(address) {
  if (!address || address === 'Não configurado') return address;
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      // A verificação de admin é feita pelo ProtectedRoute (custom claims)
      setUser(currentUser);
      setAuthStatus('');
      showMessage(`Bem-vindo, ${maskEmail(currentUser.email)}`);
  ...
  const loadMonthlyData = async (monthKey) => {
    ...
    try {
      ...
      showMessage('Erro nos dados mensais', true);
    } finally {
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-gray-100">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-3 px-4 py-2 bg-orange-500/10 border border-orange-500/20 rounded-full backdrop-blur-sm">
                  ...
                  <span className="text-sm text-gray-300">{maskEmail(user?.email)}</span>
                </div>
                <button
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
            <SummaryCard value={totalUsers.toLocaleString()} label="Usuários Registrados" />
            <SummaryCard value={totalBigPoints.toLocaleString()} label="Total BIG" />
            <SummaryCard value={monthlyBigPoints.toLocaleString()} label="Este Mês" />
            <SummaryCard value={avgPerUser.toLocaleString()} label="Média por Usuário" />
          </div>
                  ...
                  <tbody>
                    {monthlyUserData.map((user, index) => (
                      <tr key={index} className="border-b border-orange-500/10 hover:bg-orange-500/5 transition-colors">
                        ...
                        <td className="px-4 py-3 text-purple-400 font-mono text-xs">{user.walletAddress}</td>
                      </tr>
                    ))}
