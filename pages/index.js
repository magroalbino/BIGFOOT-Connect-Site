...
export default function Home() {
  ...
  // Logout — limpa apenas dados do utilizador, preserva tema e idioma
  const handleLogout = async () => {
    try {
      await signOut(auth);
      // Remove apenas dados de autenticação, preserva preferências
      ['userId', 'userEmail', 'authToken'].forEach(key => localStorage.removeItem(key));
      router.push('/');
    } catch (error) {
        ...
        <main className="relative z-10 flex-1 flex flex-col items-center text-center px-4 sm:px-6 lg:px-8">
          ...
          <section className="w-full max-w-5xl pt-24 pb-20 flex flex-col items-center">
            ...
            <div className="hero-cta flex gap-4 flex-wrap justify-center mb-10">
              <a
                href="https://github.com/fabricioricard/BIGFOOT-Connect/releases/download/v1.4.1/BIGFOOT-Connect-Setup-1.4.1.exe"
            ...
            {/* Aviso instalador — removido dangerouslySetInnerHTML */}
            <div
