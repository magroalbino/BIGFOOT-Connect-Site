...
export default function Register() {
  ...
  // Inicialização do tema (protegida contra SSR)
  useEffect(() => {
  ...
  const adicionarContatoBrevo = async (email, nome = '') => {
    try {
      await fetch(`${API_BASE_URL}/api/addcontact`, {
        method: 'POST',
        headers: {
      ...
      // Silencioso: erro no Brevo não bloqueia o registo
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 4. Adicionar ao Brevo (não bloqueia)
      adicionarContatoBrevo(user.email);
