...
export default function Dashboard() {
  ...
  // Removido o useEffect que setava o mês atual
  ...
  const initDashboard = async (user) => {
    try {
      ...
      const refToken = user.uid;
      const refLink = `${window.location.origin}/register?ref=${encodeURIComponent(refToken)}`;
  ...
  const filterDataByMonth = (month) => {
    let filtered = {};
    let periodTotal = 0, periodDays = 0;
    let isYear = false;
    ...
    if (month === 'all') {
      ...
      setMonthlyData({ ...monthlyData, showMonthly: false, isYear: false });
    } else {
      isYear = /^\d{4}$/.test(month);
      const prefix = month;
      
      Object.keys(allUsageData).forEach(date => {
        if (date.startsWith(prefix)) {
          filtered[date] = allUsageData[date];
          periodTotal += allUsageData[date];
          if (allUsageData[date] > 0) periodDays++;
        }
      });

      setMonthlyData({
        totalPoints: periodTotal,
        activeDays: periodDays,
        avgDaily: periodDays > 0 ? periodTotal / periodDays : 0,
        showMonthly: true,
        isYear,
      });
    }
  ...
  const availableMonths = useMemo(() => {
    const base = [{ value: 'all', label: t('allData') }];
    const today = new Date();
    const currentYear = today.getFullYear();
    const currentMonth = today.getMonth() + 1;

    // Ano 2025 completo
    base.push({ value: '2025', label: '2025' });
              ...
              {monthlyData.showMonthly && (
                <div className={`rounded-2xl border p-6 mb-6 ${isDark ? 'bg-orange-500/5 border-orange-500/10' : 'bg-orange-50/60 border-orange-100'}`}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    {[
                      { 
                        value: `${monthlyData.totalPoints.toFixed(2)} BIG`, 
                        label: monthlyData.isYear ? t('sharedThisYear') : t('sharedThisMonth') 
                      },
                      { 
                        value: monthlyData.activeDays, 
                        label: monthlyData.isYear ? t('activeDaysYear') : t('activeDaysMonth') 
                      },
                      { 
                        value: `${monthlyData.avgDaily.toFixed(2)} BIG`, 
                        label: monthlyData.isYear ? t('dailyAverageYear') : t('dailyAverage') 
                      },
                    ].map((stat, i) => (
